...
void displayTimeSlots(int timeSlots[], int choice12)
{
    int a=0;
...
void bookTimeSlot(int timeSlots[], int slotNumber,int choice11)
{
    ...
    if (turfs[choice11-1].timeSlots[slotNumber - 1] == 0)
    {
        ...
        strcat(verify_ID,booking_username);
        strcat(verify_ID,verification);
        verify_account_write(verify_ID);
        strcpy(verify_ID,For_empty);
...
void displayBookedSlots()
{
    ...
    else
    {
         ...
         fclose(file);                      //printf("Error reading from the file.\n");
    }
...
book()
{
    ...
    // Use fscanf's return value to check for successful reading
    while (fscanf(file, "%d %d", &choice15, &slot) == 2)
    {
    ...
    do
    {
        ...
        case 1:
            ...
            break;
        case 2:
            printf("\nEnter the time slot number you want to book: ");
            scanf("%d", &slotNumber);
            bookTimeSlot(timeSlots, slotNumber,choice1);
            break;
        case 3:
            printf("\nEnter the time slot number you want to cancel: ");
            scanf("%d", &slotNumber);
...
int user_name_exits(char *username)
{
    FILE*log;
    log=fopen("login12.txt", "r");
    struct login f;
    while(fread(&f,sizeof(f),1,log))
    {
...
create()
{
    ...
    do
    {
        printf("\n\nEnter a username: ");
        scanf("%s", l.username);
        if(user_name_exits(l.username))
        {
            printf("\nThis user is already in use. Try another.");
        }
    }
    while(user_name_exits(l.username));
